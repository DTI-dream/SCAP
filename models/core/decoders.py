...
from models.core.attention import MultiHeadAttention
from models.core.utils import sinusoid_encoding_table, PositionWiseFeedForward
from models.containers import Module, ModuleList


class SummaryForgetDecoderLayer(Module):
    def __init__(self, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1, self_att_module=None,
                 enc_att_module=None, self_att_module_kwargs=None, enc_att_module_kwargs=None):
        super(SummaryForgetDecoderLayer, self).__init__()
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        ...
        enc_att2 = self.enc_att(enc_att1, enc_output[:, 1], enc_output[:, 1], mask_enc_att) * mask_pad
        enc_att3 = enc_att2 * mask_pad
...
class SummaryForgetDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
        ...
        super(SummaryForgetDecoder, self).__init__()
        self.d_model = d_model
        self.word_emb = nn.Embedding(vocab_size, d_model, padding_idx=padding_idx)
        self.pos_emb = nn.Embedding.from_pretrained(sinusoid_encoding_table(max_len + 1, d_model, 0), freeze=True)
        self.layers = ModuleList(
            [SummaryForgetDecoderLayer(d_model, d_k, d_v, h, d_ff, dropout, self_att_module=self_att_module,
